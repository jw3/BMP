...
<xsl:template match="BMP" mode="M1" priority="1000">
    <svrl:fired-rule context="BMP" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="Identifier = ('BM', 'BA', 'CI', 'CP', 'IC', 'PT')" />
      <xsl:otherwise>
        <svrl:failed-assert test="Identifier = ('BM', 'BA', 'CI', 'CP', 'IC', 'PT')">
...
<xsl:template match="Reserved" mode="M2" priority="1000">
    <svrl:fired-rule context="Reserved" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="xs:integer(.) eq 0" />
      <xsl:otherwise>
        <svrl:failed-assert test="xs:integer(.) eq 0">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>
...
<xsl:template match="Number_of_Color_Planes" mode="M5" priority="1000">
    <svrl:fired-rule context="Number_of_Color_Planes" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="xs:integer(.) eq 1" />
      <xsl:otherwise>
        <svrl:failed-assert test="xs:integer(.) eq 1">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>
...
<xsl:template match="Number_of_Bits_per_Pixel" mode="M6" priority="1002">
    <svrl:fired-rule context="Number_of_Bits_per_Pixel" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="xs:integer(.) = (1, 4, 8, 16, 24, 32)" />
      <xsl:otherwise>
        <svrl:failed-assert test="xs:integer(.) = (1, 4, 8, 16, 24, 32)">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>
...
<xsl:template match="BITMAPCOREHEADER" mode="M6" priority="1001">
    <svrl:fired-rule context="BITMAPCOREHEADER" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="not(xs:integer(Number_of_Bits_per_Pixel) = (16, 32))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(xs:integer(Number_of_Bits_per_Pixel) = (16, 32))">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>
...
<xsl:template match="*[Number_of_Colors_in_the_Color_Palette]" mode="M6" priority="1000">
    <svrl:fired-rule context="*[Number_of_Colors_in_the_Color_Palette]" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="xs:integer(Number_of_Colors_in_the_Color_Palette) le math:pow(2, Number_of_Bits_per_Pixel)" />
      <xsl:otherwise>
...
<xsl:template match="Compression_Method[. eq 'RLE 8-bit/pixel']" mode="M7" priority="1001">
    <svrl:fired-rule context="Compression_Method[. eq 'RLE 8-bit/pixel']" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="xs:integer(../Number_of_Bits_per_Pixel) eq 8" />
      <xsl:otherwise>
        <svrl:failed-assert test="xs:integer(../Number_of_Bits_per_Pixel) eq 8">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>
...
<xsl:template match="Compression_Method[. eq 'RLE 4-bit/pixel']" mode="M7" priority="1000">
    <svrl:fired-rule context="Compression_Method[. eq 'RLE 4-bit/pixel']" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="xs:integer(../Number_of_Bits_per_Pixel) eq 4" />
      <xsl:otherwise>
        <svrl:failed-assert test="xs:integer(../Number_of_Bits_per_Pixel) eq 4">
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text>
